...
def test_tip_mass_zero_matches_continuous_formula():
    f_continuous = natural_frequency_cantilever(
        GEOMETRY, young_modulus_pa=200e9, density_kg_m3=7850.0
    )
...
def test_tip_mass_lowers_frequency():
    f_no_tip = natural_frequency_cantilever(
        GEOMETRY, young_modulus_pa=200e9, density_kg_m3=7850.0
    )
    f_tip = natural_frequency_cantilever(
        GEOMETRY, young_modulus_pa=200e9, density_kg_m3=7850.0, tip_mass_kg=0.0003
    )
...
def test_tip_mass_roundtrip_recovers_modulus():
    young_true = 200e9
    tip = 0.00035
    f = natural_frequency_cantilever(
        GEOMETRY, young_modulus_pa=young_true, density_kg_m3=7850.0, tip_mass_kg=tip
    )
    recovered = young_modulus_from_frequency(
        GEOMETRY, frequency_hz=f, density_kg_m3=7850.0, tip_mass_kg=tip
    )
